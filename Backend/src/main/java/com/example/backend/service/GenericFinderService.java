...
public class GenericFinderService {
    ...
    /**
     * Get salt composition for a branded medicine
     * First checks local database (BRAND_SALT_MAP), normalized matches, and falls back to web scraping
     */
    private List<String> getVerifiedSaltComposition(String brandName) {
        List<String> salts = getSaltComposition(brandName);
        ...
        if (!normalizedName.isEmpty() && !normalizedName.equalsIgnoreCase(brandName.trim())) {
            System.out.println("[GenericFinder] 🔄 Retrying web scraping with normalized name: " + normalizedName);
    ...
    /**
     * Get salt composition using multi-stage strategy:
     * 1. Direct local BRAND_SALT_MAP lookup
     * 2. Normalized brand name lookup (removing strength/dosage/form)
     * 3. Partial key match in BRAND_SALT_MAP
     * 4. Check if brand name itself contains a generic salt name
     * 5. Web scraping from 1mg.com
     */
    public List<String> getSaltComposition(String brandName) {
        ...
        String rawLower = brandName.trim().toLowerCase();
        String normalizedName = normalizeBrandName(brandName);

        // 1. Direct key match
        if (BRAND_SALT_MAP.containsKey(rawLower)) {
            System.out.println("[GenericFinderService] 🔎 Direct match in BRAND_SALT_MAP: " + rawLower);
            return BRAND_SALT_MAP.get(rawLower);
        }
        
        // 2. Normalized name match
        if (!normalizedName.isEmpty() && BRAND_SALT_MAP.containsKey(normalizedName)) {
            System.out.println("[GenericFinderService] 🔎 Normalized match in BRAND_SALT_MAP: " + normalizedName);
            return BRAND_SALT_MAP.get(normalizedName);
        }

        // 3. Partial / prefix key match in BRAND_SALT_MAP
        for (Map.Entry<String, List<String>> entry : BRAND_SALT_MAP.entrySet()) {
            String key = entry.getKey();
            if (rawLower.startsWith(key) || key.startsWith(rawLower) || 
                (!normalizedName.isEmpty() && (normalizedName.startsWith(key) || key.startsWith(normalizedName)))) {
                System.out.println("[GenericFinderService] 🔎 Partial match in BRAND_SALT_MAP ('" + key + "') for: " + brandName);
                return entry.getValue();
            }
        }

        // 4. Check if brand name itself contains a common salt name
        String[] commonSalts = {
            "paracetamol", "ibuprofen", "aceclofenac", "diclofenac", "cetirizine", "levocetirizine",
            "montelukast", "azithromycin", "amoxicillin", "cefixime", "ciprofloxacin", "norfloxacin",
            "metronidazole", "doxycycline", "omeprazole", "pantoprazole", "ranitidine", "dicyclomine",
            "drotaverine", "metformin", "glimepiride", "amlodipine", "telmisartan", "atenolol",
            "metoprolol", "losartan", "atorvastatin", "rosuvastatin", "calcium", "vitamin d3", "multivitamin",
            "ondansetron", "domperidone", "alprazolam", "aspirin", "nimesulide", "levothyroxine", "salbutamol"
        };
        for (String saltName : commonSalts) {
            if (rawLower.contains(saltName)) {
                System.out.println("[GenericFinderService] 💡 Detected salt name in input query: " + saltName);
                return List.of(saltName);
            }
        }

        // 5. Fallback to web scraping
        System.out.println("[GenericFinderService] 🌐 Web scraping fallback for salt: " + brandName);
        List<String> scrapedSalts = scrapeFromWeb(brandName);
        if (scrapedSalts != null && !scrapedSalts.isEmpty()) {
            return scrapedSalts;
        }

        if (!normalizedName.equals(rawLower)) {
            scrapedSalts = scrapeFromWeb(normalizedName);
            if (scrapedSalts != null && !scrapedSalts.isEmpty()) {
                return scrapedSalts;
            }
    ...
    private String normalizeBrandName(String brandName) {
        ...
        return brandName.trim()
                .toLowerCase()
                .replaceAll("\\s+", " ")
                .replaceAll("\\d+\\s*(mg|ml|gm|g|mcg|iu)$", "")
                .replaceAll("[^a-z0-9\\s-]", "")
                .trim();
    }
    ...
    private List<Medicine> searchGenericsBySalt(List<String> salts) {
        ...
        try {
            ...
            if (medicineRepository != null) {
                for (String salt : normalizedSearchSalts) {
                    try {
                        ...
                        // ignore repository errors in offline mode
                    }
                }
            }
            ...
            System.out.println("[GenericFinder] 📦 csvMatches count from MedicineService.findBySalts: " + (csvMatches != null ? csvMatches.size() : 0));
            if (csvMatches != null) {
            ...
            System.out.println("[GenericFinder] 📊 Safe matches count: " + exactMatches.size());
            
            List<Medicine> results;
            if (!exactMatches.isEmpty()) {
                results = exactMatches;
            } else if (!candidates.isEmpty()) {
                System.out.println("[GenericFinder] ℹ️ Using candidate matches fallback (" + candidates.size() + " candidates)");
                results = new ArrayList<>(candidates);
            } else {
                System.out.println("[GenericFinder] ⚠️ No matches found for salts");
                return new ArrayList<>();
            }
            ...
            System.out.println("[GenericFinder] 🏁 final results count: " + results.size());
            for (Medicine r : results) {
                System.out.println("[GenericFinder]   -> " + r.getName() + " (Rs " + r.getPrice() + ")");
            }
