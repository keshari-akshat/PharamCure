...
const GenericMedicineInfo = () => {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      // Call backend API to find generic alternatives for branded medicine
      const res = await axios.get(`/api/find-generic?brandName=${encodeURIComponent(search.trim())}`);
      const data = res.data;
      if (data && data.success && Array.isArray(data.genericAlternatives) && data.genericAlternatives.length > 0) {
        const mapped = data.genericAlternatives.map(med => ({
          name: med.name,
          company: med.genericSource || 'Generic',
          mrp: med.price || 0,
          composition: med.salts ? med.salts.join(' + ') : (data.saltComposition ? data.saltComposition.join(' + ') : 'N/A')
        })).sort((a, b) => a.mrp - b.mrp);
        setResults(mapped);
      } else {
        setResults([]);
        setError(data.message || "No generic alternative found for this medicine.");
      }
    } catch (err) {
      setResults([]);
